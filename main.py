...
def main(argv=None):
            ...
            for it in wb["主表"]["R"]:
                if (str(it.value).find(cell.value) >= 0):
                    班级 = str(wb["主表"].cell(it.row, 6).value) +\
                        str(wb["主表"].cell(it.row, 7).value)
            dict2["详情"].append(
                [班级, cell.value, wb["学生接触外省人员登记"].cell(cell.row, 4).value])
            ...
            for it in wb["主表"]["R"]:
                if (str(it.value).find(cell.value) >= 0):
                    班级 = str(wb["主表"].cell(it.row, 6).value) +\
                        str(wb["主表"].cell(it.row, 7).value)
            dict3["详情"].append(
                [班级, cell.value, wb["家长接触重点疫区、境外登记"].cell(cell.row, 4).value])
    ...
    wbNEW["综合统计"].append(
        ["学校名称", "应到人数", "实到人数", "请假学生登记人数总数", "因发热未到或请假", "因咳嗽腹泄乏力请假人数", "因其他原因请假人数",
         ...
         "学生接触外省人员登记人数总数", "家长接触重点疫区、境外登记人数总数", "乘坐公交车学生人数"])
